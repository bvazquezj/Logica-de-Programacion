...
int bol[20], c1[20], c2[20], c3[20], c4[20], c5[20], c6[20], c7[20], prom[20], i, aux;
...
Leer(){
	if (fp=fopen(("calif.txt","r") == NULL)){
		...
		promedio[i]=(c1[i]+c2[i]+c3[i]+c4[i]+c5[i]+c6[i]+c7[i])/7;
		}
	}
}
Ordenar(){ 
...
grabar_archivo()
	if (fp=fopen("salida.txt","w")==NULL){
		...
		for (i=0;i<20; i++){
			...
			fprintf(fp,”Nombre:   %s    %s  %s              bol. %d“, nombre[i],ap[i],am[i],bol[i]);
			fprintf(fp, "___________________________________\n");
